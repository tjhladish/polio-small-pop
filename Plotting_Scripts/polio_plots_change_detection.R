...
full_detect_3500 = read.table(paste0(dir,'e_and_d_values-0pcase_filter_det_1_N_3500_beta_135_fast.out'), col.names=c('time', 'E&D'))
...
full_detect_3500_filter = read.table(paste0(dir,'e_and_d_values-1pcase_filter_det_1_N_3500_beta_135_fast.out'), col.names=c('time', 'E&D'))
...
full_detect_10000 = read.table(paste0(dir,'e_and_d_values-0pcase_filter_det_1_N_10000_beta_135_fast.out'), col.names=c('time', 'E&D'))
half_detect_10000 = read.table(paste0(dir,'e_and_d_values-0pcase_filter_det_50_N_10000_beta_135_fast.out'), col.names=c('time', 'E&D'))
quarter_detect_10000 = read.table(paste0(dir,'e_and_d_values-0pcase_filter_det_25_N_10000_beta_135_fast.out'), col.names=c('time', 'E&D'))

full_detect_10000_filter = read.table(paste0(dir,'e_and_d_values-1pcase_filter_det_1_N_10000_beta_135_fast.out'), col.names=c('time', 'E&D'))
half_detect_10000_filter = read.table(paste0(dir,'e_and_d_values-1pcase_filter_det_50_N_10000_beta_135_fast.out'), col.names=c('time', 'E&D'))
quarter_detect_10000_filter = read.table(paste0(dir,'e_and_d_values-1pcase_filter_det_25_N_10000_beta_135_fast.out'), col.names=c('time', 'E&D'))


marker = list(color = brewer.pal(8,"GnBu"))

png(paste0(dir,'Effect_of_detection_rate_on_ED_statistic_N_20000_ED_pres.png'), width=1400, height=800, res=150)
...
png(paste0(dir,'Effect_of_detection_rate_on_ED_statistic_N_3500_ED_pres.png'), width=1400, height=800, res=150)
...
png(paste0(dir,'Effect_of_detection_rate_on_ED_statistic_N_25000_ED_pres.png'), width=1400, height=800, res=150)
plot(full_detect_25000, type='l',lty=1, xlab='Time since last paralytic case (years)', ylab='Endemic potential statistic (probability of circulation)',xlim=c(0,10),col=marker$color[5])
lines(full_detect_25000_filter,type='l', col=marker$color[5],lty=4)
lines(half_detect_25000,type='l', col=marker$color[6])
lines(half_detect_25000_filter,type='l', col=marker$color[6],lty=4)
lines(quarter_detect_25000,type='l', col=marker$color[7])
lines(quarter_detect_25000_filter,type='l', col=marker$color[7],lty=4)
legend('topright', legend = c('ICA, 100% detection (N=25,000)','no ICA, 100% detection (N=25,000)', 'ICA, 50% detection (N=25,000)','no ICA, 50% detection (N=25,000)',
                              'ICA, 25% detection (N=25,000)', 'no ICA, 25% detection (N=25,000)'),col=c(marker$color[5],marker$color[5],marker$color[6],marker$color[6],marker$color[7],marker$color[7]),lty=c(1,4,1,4,1,4),lwd=2, bty='n' )

dev.off()

png(paste0(dir,'Effect_of_detection_rate_on_ED_statistic_N_10000_ED_pres.png'), width=1400, height=800, res=150)
